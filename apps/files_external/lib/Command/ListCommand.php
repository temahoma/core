...
class ListCommand extends Base {
	...
	public function listMounts($userId, array $mounts, InputInterface $input, OutputInterface $output) {
		...
		if ($outputType === self::OUTPUT_FORMAT_JSON || $outputType === self::OUTPUT_FORMAT_JSON_PRETTY) {
			...
			$countInvalid = 0;
			// In case adding array elements, only add them after the first two (Mount ID/Point)
			...
			// Removes all non used printable array keys for option -s. Only the first two and the last one is kept.
			// This leaves the above array construction in it´s originally state.
			if ($shortView) {
				$newRows = [];
